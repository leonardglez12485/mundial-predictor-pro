import { c as createLucideIcon } from "./check-CUGRap4B.mjs";
...
const SPECIAL_PREDICTION_DEADLINE_ISO = "2026-07-01T00:00:00.000Z";
...
function calculatePoints(prediction, match) {
  ...
  if (prediction.homeGoals === match.result.homeGoals && prediction.awayGoals === match.result.awayGoals)
    pts += 5;
  const actualScorers = [
    ...match.result.scorerEntries.length > 0 ? match.result.scorerEntries : match.result.scorers.map((name) => ({ name }))
  ];
  for (const s of prediction.scorers) {
    const idx = actualScorers.findIndex(
      (actualScorer) => scorerEntriesMatch(actualScorer, parseScorerEntry(s))
    );
    if (idx >= 0) {
      pts += 2;
