  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8 lg:py-10">
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight sm:text-4xl">Máximos goleadores</h1>
          <p className="mt-1 text-muted-foreground">
            Conteo automático según los goleadores cargados por el administrador en cada resultado.
          </p>
        </div>
            ...
            <div className="divide-y">
              {scorers.length === 0 ? (
                ...
                scorers.map((scorer, index) => (
                  <div
                    key={`${scorer.teamCode ?? "na"}-${scorer.name}-${index}`}
                    className="grid grid-cols-[72px_minmax(0,1fr)_140px_92px] items-center gap-3 px-5 py-3"
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`flex h-9 w-9 items-center justify-center rounded-lg text-sm font-bold ${index < 3 ? "bg-primary/10 text-primary-deep" : "bg-secondary text-muted-foreground"}`}
                      >
                        {index + 1}
                      </div>
                    ...
                    <div className="flex min-w-0 items-center gap-2 text-sm text-muted-foreground">
                      {scorer.teamCode && teamMap.get(scorer.teamCode) ? (
                        <>
                          <Flag
                            team={teamMap.get(scorer.teamCode)!}
                            size={16}
                    ...
                    <div className="text-right text-lg font-bold text-primary-deep tabular-nums">
                      {scorer.goals}
                    </div>
                  </div>
                ))
